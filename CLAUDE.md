...
6. There is no concept of withdrawal or adding funds. Funds are only used when game invites are accepted and automatically disbursed upon game end. Do not add deposit/withdraw UI.
7. When debugging, always add console.log statements to trace the issue, check the logs (`logs/web.log`, `logs/backend.log`), and iterate. Do not guess at fixes — use logs to confirm what is actually happening before changing code.
...
9. Never delete `training/runs/v1` (or any training run folder) unless the user explicitly asks to delete it. When restarting training, use `--resume` by default.
...
- Dev command: `pnpm dev` (overmind → web + backend)
- Logs: `logs/web.log`, `logs/backend.log`
...
- `src/pages/OnboardingPage.tsx` — ToS step only
- `src/pages/HomePage.tsx` — create game, balance
...
- `prisma/schema.prisma` — models: User, Charity, Game, Contribution

## Key env vars
- Backend: DATABASE_URL, CLERK_PUBLISHABLE_KEY, CLERK_SECRET_KEY, FRONTEND_URL, PORT
- Frontend: VITE_CLERK_PUBLISHABLE_KEY, VITE_API_URL, VITE_WS_URL
